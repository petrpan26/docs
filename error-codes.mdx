...
All API endpoints return errors in JSON format with a `detail` field:
...
<ResponseField name="402" type="Payment Required">
  ...
  **Solution:** Add more credits to your account in the [dashboard](https://portal.viggle.ai/dashboard).
</ResponseField>
...
<ResponseField name="429" type="Rate Limited">
  ...
  **Common causes:**
  - Exceeding your organization's concurrency limit (default: 5 concurrent render jobs)

  **Solution:** Wait for existing jobs to complete, or implement exponential backoff and retry logic.
</ResponseField>
...
| Rate limited | 429 | Wait for jobs to finish or retry with backoff |
