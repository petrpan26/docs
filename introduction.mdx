...
curl -X POST "https://apis.viggle.ai/api/scenes/preprocess" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -F "video=@dance.mp4" -F "name=V3 Dance" -F "model=V3_Preview"
...
<Warning>A scene preprocessed on V3 cannot be rendered with V4 (and vice versa). The API returns an error if the models don't match.</Warning>
