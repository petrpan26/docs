...
<Tabs>
  <Tab title="On-Demand" id="on-demand" icon="zap">
    ...
    while requests.get(f"{BASE}/api/render/{job['job_id']}").json()["status"] != "complete":
        time.sleep(5)
    r = requests.get(f"{BASE}/api/render/{job['job_id']}/download", headers=h)
    ...
    const video = await fetch(`${BASE}/api/render/${job.job_id}/download`, { headers: h });
  ...
  <Tab title="Preprocessed" id="preprocessed" icon="rocket">
    Create character + scene once (free), then render with IDs for lower latency.
  ...
  <Tab title="Live Stream" id="live-stream" icon="radio">
    Long-poll `/poll` for fMP4 chunks as they render—ideal for previews and streaming.

    ```python
    import requests
    API_KEY, BASE, job_id = "YOUR_API_KEY", "https://apis.viggle.ai", "JOB_ID"
    h = {"Authorization": f"Bearer {API_KEY}"}
    last_chunk = -1
    while True:
        resp = requests.get(f"{BASE}/api/render/{job_id}/poll", headers=h,
            params={"last_chunk": last_chunk, "timeout": 30}).json()
        for chunk in resp["new_chunks"]:
            print(chunk["chunk_id"], chunk["cdn_url"])
    ...
    <Card title="Live Rendering Guide" icon="play" href="/guides/live-render">
      MSE playback, long-polling, and fMP4 details
    </Card>
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Pick a mode" icon="list">
    Use the **Rendering modes** tabs: **On-Demand** (fastest first render), **Preprocessed** (~3x faster at scale), **Import Templates**, or **Live Stream**.
  </Step>
  <Step title="Render" icon="film">
    `POST /api/render` — poll `GET /api/render/{job_id}` until `complete`, then download.
  </Step>
</Steps>
...
<Columns cols={3}>
  <Card title="Discord Community" icon="message-circle" href="https://discord.com/invite/viggle">
