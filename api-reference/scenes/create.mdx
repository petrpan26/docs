...
You can optionally pass `background_mode` and `model` to configure the scene at creation time.

<Warning>
  **Model binding:** Each scene is locked to the model specified at creation (default `V4_Preview`). Attempting to render a scene with a different model returns error `CS-UORC01-013 scene_model_mismatch`. Create separate scenes for each model if needed.
</Warning>
