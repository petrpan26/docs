...
```bash cURL
curl "https://api.viggle.ai/api/characters/char_550e8400e29b41d4" \
...
while True:
    response = requests.get(
        f"https://api.viggle.ai/api/characters/{character_id}",
...
async function waitForCharacter() {
  while (true) {
    const response = await fetch(
      `https://api.viggle.ai/api/characters/${characterId}`,
