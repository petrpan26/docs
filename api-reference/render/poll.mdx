...
```bash cURL
curl "https://api.viggle.ai/api/render/job_abc123xyz/poll?last_chunk=-1&timeout=30" \
...
while (true) {
  const response = await fetch(
    `https://api.viggle.ai/api/render/${jobId}/poll?last_chunk=${lastChunk}&timeout=30`,
...
```json Rendering
{
  ...
  "progress": { "completed_chunks": 2, "total_chunks": 5 }
}
```
...
```json Queued
{
  ...
  "queue_position": 3,
  "new_chunks": [],
  "progress": { "completed_chunks": 0, "total_chunks": 5 }
}
```
...
```json Complete
{
  ...
  "cdn_url": "https://cdn.viggle.ai/videos/..."
}
```
